...
int main(int argc, char* argv[]) {
	...
	// measures are in [m]
		...
		ChVector<> COG_link(3.45, 0.0, 0.5);
		ChVector<> COG_bucket(4.56,.0, 0.525);
		...
		ChVector<> POS_lift2lever(2.25, 0, 2.1);//end position of actuator lift->lever
		...
		ChVector<> POS_link2bucket(4.35, .0, 1.21);//chassis piston(BUCKET LEVER) insertion abs frame
	...
	// 2 Create the rigid bodies
		...
		// LINK
						...
						link->SetRot(rot4);
						link->SetMass(2.5);
						link->SetInertiaXX(ChVector<>(1., 1., 1.));
						// No collision properties:
						// visualization properties:
...
// No way
						auto bp_asset = std::make_shared<ChLineShape>();//asset
						bp_asset->GetLineGeometry()->GetEndA() = POS_lift2lever;//asset
						bp_asset->GetLineGeometry()->GetEndB() = PIS_lift2lever;//asset
